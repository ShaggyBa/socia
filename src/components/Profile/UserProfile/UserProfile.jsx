...
const UserProfile = (props) => {
    ...
    return (
        <div className={`${s.user__profile} block`}>
                ...
                {props.isOwner &&
                    <div className={s["profile--actions"]}>
                        ...
                        <div className={isMenuOpen ? s["profile--menu-open"] : s["profile--menu"]}>
                            <input className={s.uploadImage} type="file" placeholder={"Upload your image"}
            ...
            <div className={s.user__content}>
                <div className={s.user__name}>{props.profile?.fullName || "no name"}</div>
                <ProfileStatus status={props.status} updateStatus={props.updateStatus}/>
                <div
                    className={s.user__status}>{props.profile?.lookingForAJob ? "looking for a job" : "not looking for a job"}</div>
                <div className={s.user__info}>{props.profile?.lookingForAJobDescription || "no description"}</div>
            </div>
        </div>
    )
}
